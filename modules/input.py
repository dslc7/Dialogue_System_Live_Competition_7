...
class AIN(RemdisModule):
    def __init__(self, pub_exchanges=["ain"]):
        config_filename = (
            Path(sys.argv[0]).resolve().parent / ".." / "config" / "config.yaml"
        )
        super().__init__(
        ...
        self.device_index = self.config["AIN"]["device_index"]
...
class VIN(RemdisModule):
    def __init__(self, pub_exchanges=["vin"]):
        ...
        self.device_index = self.config["VIN"]["device_index"]
...
if __name__ == "__main__":
    # dockerコンテナ内ではなく、ローカル環境から実行されるべき
    main()
