...
class TextVAP(RemdisModule):
    def __init__(self, pub_exchanges=["bc", "vap", "emo_act"], sub_exchanges=["asr"]):
        ...
        self.max_verbal_backchannel_num = self.config["TEXT_VAP"][
            "max_verbal_backchannel_num"
        ]
        self.max_nonverbal_backchannel_num = self.config["TEXT_VAP"][
            "max_nonverbal_backchannel_num"
        ]
        ...
        while True:
            ...
            # ADD/COMMITの場合は応答候補生成
                ...
                t = threading.Thread(
                    ...
                    args=(input_iu["timestamp"], user_utterance),
                )
                t.start()
    ...
    def parse_line_for_text_vap(self, line):
        ...
        text_vap_score = (
            int(text_vap_completion) if text_vap_completion.isdigit() else 0
        )
        ...
        for chunk in self.response:
            ...
            if "content" in chunk_message.keys():
                ...
                if current_completion_line.startswith(("a", "==a")):
                    label, content = self.parse_line_for_backchannel(
                        current_completion_line
                    )
                    if label:
                ...
                # 表出すべき感情の判定
                    ...
                    if label:
                        nonverbal_backchannel["expression"] = (
                            MMDAgentEXLabel.id2expression[label]
                        )
                ...
                # 表出すべき動きの判定
                    ...
                    if label:
                        nonverbal_backchannel["action"] = MMDAgentEXLabel.id2action[
                            label
                        ]
                    if nonverbal_backchannel:
                        self.log(
                            f"***** BACKCHANNEL: {query=} {nonverbal_backchannel=} *****"
                        )
                ...
                # 応答を確定させるかどうかの判定
                    ...
                    if triggered:
                        self.log(
                            f"***** TEXT_VAP: {query=} {current_completion_line=} *****"
                        )
        ...
        with self.backchannel_sent_lock:
            ...
            if "bc" in content:
                if (
                    ...
                    and self.sent_verbal_backchannel_counter
                    < self.max_verbal_backchannel_num
                ):
                    self.last_verbal_backchannel_timestamp = asr_timestamp
                ...
                if (
                    ...
                    and self.is_listening
                    and self.sent_nonverbal_backchannel_counter
                    < self.max_nonverbal_backchannel_num
                ):
                    self.last_nonverbal_backchannel_timestamp = asr_timestamp
