...
class TimeOut(RemdisModule):
    def __init__(self, pub_exchanges=["vap", "asr"], sub_exchanges=["asr", "tts"]):
    ...
    def check_timeout(self):
        while True:
            ...
            with self.lock_for_utterance_timestamp:
                ...
                if (
                    current_time - self.last_utterance_timestamp
                    > self.max_silence_time * (self.timeout_num + 1)
                ):
                    self.timeout_num += 1
    ...
    def send_system_take_turn(self):
        iu = self.createIU("（沈黙）", "str", RemdisUpdateType.ADD)
        self.printIU(iu)
        self.publish(iu, "asr")
        time.sleep(0.5)
    ...
    def callback_tts(self, ch, method, properties, in_msg):
        current_time = time.time()
        if (
            self.last_timeout_timestamp is not None
            and current_time - self.last_timeout_timestamp < self.block_time
        ):
            return

        in_msg = self.parse_msg(in_msg)
        # Azure APIで出力されるtimestampと現在時刻に齟齬がある
        # そのため現在時刻を明示的に指定
        in_msg["timestamp"] = time.time()
        self.input_iu_buffer.put(in_msg)
