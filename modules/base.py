...
class RemdisModule:
    ...
    def publish(self, message, exchange):
        try:
            self.pub_connections[exchange]["channel"].basic_publish(
                exchange=exchange, routing_key="", body=json.dumps(message)
            )
        except Exception as e:
            print(e)
            print(f"Try to re-make connection: {exchange}")
            self.pub_connections[exchange] = self.mk_pub_connection(exchange)
            print("Try to re-send message")
            self.pub_connections[exchange]["channel"].basic_publish(
                exchange=exchange, routing_key="", body=json.dumps(message)
            )
            print(f"Recovered pub:{exchange}")

    # 汎用メッセージ受信関数
    def subscribe(self, exchange, callback):
        self.sub_connections[exchange]["channel"].basic_consume(
            queue="", auto_ack=True, on_message_callback=callback
        )
    ...
    def printIU(self, iu, flush=False):
        sys.stdout.write(
            "[%s] Body: %s, Update_type: %s, ID: %s\n"
            % (iu["timestamp"], iu["body"], iu["update_type"], iu["id"])
        )
        if flush:
            sys.stdout.flush()
...
class RemdisUtil:
    def remove_revoked_ius(self, iu_buffer):
        revoked_iu_ids = [
            iu["id"] for iu in iu_buffer if iu["update_type"] == RemdisUpdateType.REVOKE
        ]
